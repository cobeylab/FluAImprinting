...
source('imprinting_function_with_vaccination.R')

result_folder="../results_vac_imprinting/" # Output folder for results
...
raw_imprinting_v = calc_imprinting_all(attack_rate)
m_h1_v = m_h2_v = m_h3_v = m_v = m_n_v = m_v_mono = template
for (season in seasons){
    for (birth_year in seq(1918, 2017)){
        if (birth_year <= season_bounds[as.character(season), 'max_year'] &
            ...
            imps = raw_imprinting_v %>% filter(Birth_year == birth_year, Season == season)
            m_h1_v[as.character(season), as.character(birth_year)] = imps$H1
            m_h2_v[as.character(season), as.character(birth_year)] = imps$H2
            m_h3_v[as.character(season), as.character(birth_year)] = imps$H3
            m_v[as.character(season), as.character(birth_year)] = imps$vac
            m_v_mono[as.character(season), as.character(birth_year)] = imps$panvac
            m_n_v[as.character(season), as.character(birth_year)] = 1 - (imps$H3 + imps$H1 + imps$H2 + imps$vac + imps$panvac)
        }
    }
}
