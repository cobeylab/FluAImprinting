calc_cAIC <- function(model_neg_log_lik, n, K) {
    aic <- -2*(-model_neg_log_lik) + 2*K + ((2*K*(K+1))/(n-K-1))
    return(aic)
}


model_lik <- function(pp) {

    storage = vector('numeric', nrow(inc_data$h1_unvac))
    ...
    for(i in 1:nrow(incH1_unvac)){
        ...
        if (rownames(incH1_unvac)[i] != 2011){
          storage[i] = -(lik_h1 + lik_h3)
        } else{
          print(rownames(incH1_unvac)[i])
        }
    }
    lk = sum(storage)
    return(lk) 
...
get_age_contribution = function(pars, p0){
